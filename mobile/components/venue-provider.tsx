/**
 * Venue and session state for the native client.
 *
 * A near-verbatim port of the web client's provider. It moves unchanged because
 * it is plain React — hooks, context, useMemo — with no DOM anywhere in it. The
 * only differences are the import paths and the absence of Next's 'use client'
 * directive, which React Native has no equivalent of.
 *
 * That it ports this cleanly is the shared architecture paying out: every
 * decision this provider makes — what rehydrates, what persists, what the
 * workout derives from — is made in shared src/, not here.
 *
 * The workout is still derived and never stored, so the same seed and the same
 * confirmed inventory regenerate the same session. A reload cannot hand back a
 * plausible-looking different workout, on either platform.
 *
 * Keeping this in step with the web provider is a review obligation for now.
 * If the two drift, the sharable parts belong in src/.
 */
...
import type { LoadOutcome } from '../lib/venue-store.ts';
import type {
  CandidateFeature,
...
import { clearSession, readSession, writeSession } from '../lib/session-store.ts';
import type { SessionRecord, SessionSummary } from '../lib/session-store.ts';
