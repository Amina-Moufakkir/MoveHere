...
**Version 4.6 — Park-first, venue-aware, evidence-driven**
...
>
> **Supersedes v4.5**, which is retained unchanged as the historical product authority for that version. v4.6 changes no invariant, no gate and no current-stage exclusion. It records how the approved design anchor's capabilities are **classified** now that the anchor is treated as full-product direction (§23): three promotions to PLANNED, one explicit refusal, one renaming, and one capability recognised as already CURRENT. Everything §21 excludes today, it still excludes.
>
> **Nothing in §23 authorizes anything.** §22's governing rule applies to every capability it names: planned capability does not become production copy or affordance before its minimum truthful implementation exists. The words wait for the implementations.
>
> v4.4 and v4.3 remain the historical authorities for their own versions. Each version's record of what it changed is preserved in that version's own file.
...
- **Progress tracking, session history, and any claim of measured improvement.** Completion is recorded locally for one session. Nothing recorded supports a progress claim (§18).
  - **Progress and session history are PLANNED** (§23.1) and remain excluded from *this stage*. **Measured improvement is excluded from the product**, not merely from this stage — MoveHere records no load, no completed repetitions and no effort, so the claim has nothing to rest on.
...
### 23.1 Progress and session history — PLANNED
...
|---|---|---|
| §23.1 progress and history | Completed sessions, minutes programmed, sessions per week, goal and substitute distribution, session history | Getting stronger, improving fitness, performance improvement, improvement trend lines |
