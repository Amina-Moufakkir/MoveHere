...
**Version 4.7 — Park-first, venue-aware, evidence-driven**

> **Working name:** MoveHere. Availability and trademark clearance have not yet been completed.
>
> **Status:** Web Product Consolidation (§21) is the current implementation stage. **Activity and session history (§24) is the next authorized stage** and is scoped, not built. The Native Mobile Client (§20) is **paused at a validated implementation checkpoint**, not complete: it did not reach parity, and it went beyond parity in several respects. The Phase 0 foundation (§19) is mechanism-complete and released as the web MVP (`web-mvp-v1`); its remaining exit conditions are open and carried forward. Formal customer interviews and commercial validation are intentionally deferred. Product hypotheses remain explicitly unvalidated.
>
> **Supersedes v4.6**, which is retained unchanged as the historical product authority for that version. v4.7 changes no invariant that v4.6 set, removes no gate, and widens no current-stage exclusion. It authorizes one capability as the next stage and writes its contract: §24 promotes **Activity and session history** from PLANNED to **NEXT-STAGE AUTHORIZED**, records the product loop as **Environment → Prepare → Train → Record → Return**, and fixes the completed-session record as an **immutable local snapshot**. It adds six invariants (§24.16) and one documented divergence from an existing storage rule (§24.12).
>
> **§24 authorizes a stage, not a vocabulary.** §22's governing rule continues to apply in full: planned capability does not become production copy or affordance before its minimum truthful implementation exists. Activity may be built; Activity may not be advertised, linked, or hinted at until it works. Everything §21 excludes today, it still excludes, and every refusal in §23 stands unchanged — **Progress is still refused, not renamed.**
>
> v4.5, v4.4 and v4.3 remain the historical authorities for their own versions. Each version's record of what it changed is preserved in that version's own file.
...
|---|---|---|
| §24 Activity and history | Completed sessions, sessions per week, activity dates, goal and substitute distribution, session history. **Minutes programmed is permitted but not shipped in Activity v1 (§24.11).** | Getting stronger, improving fitness, performance improvement, improvement trend lines, streaks (§24.10) |
...
**SAFE DERIVATIONS.** Completed sessions in total; completed sessions this week;
sessions per week; activity dates; goal distribution; park versus substitute
distribution; movement count per session; number of distinct dates with a
record.
...
when a persistent Activity destination lists completed sessions, marks an
activity calendar, and reports completed sessions this week under the definitions
in §24.9; when a record can be opened in full and deleted in whole; when history
